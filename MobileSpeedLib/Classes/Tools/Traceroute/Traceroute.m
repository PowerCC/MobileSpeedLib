...
+ (instancetype)startTracerouteWithHost:(NSString *)host
                           ...
                           stepCallback:(TracerouteStepCallback)stepCallback
                                 finish:(TracerouteFinishCallback)finish {
    Traceroute *traceroute = [[Traceroute alloc] initWithHost:host port:port maxTtl:kTraceMaxJump stepCallback:stepCallback finish:finish];
    if (queue != nil) {
...
- (void)run {
    ...
    if (_port == nil || _port.length == 0) {
        _port = [NSString stringWithFormat:@"%d", kTraceRoutePort];
    }
